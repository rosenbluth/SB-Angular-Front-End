...
if ( $mapWrapper.length > 0 ) {
	var map = new GMaps({
		div: '#map',
		lat : 30.2661141, // Provide your longtitude
		lng : -97.7521338,  // Provide your longtitude
		scrollwheel: false,
	...
	map.addMarker({
		...
		icon: 'img/marker.png',
		infoWindow: {
			content: '<p>SandBox Commerce is located on the second floor in Galvanize.</p>' // Provide your address to show on pop up
		}
	});
}
	...
	$('.mailchimpForm').ajaxChimp({
		callback: function(resp) {
			...
			if (resp.result === 'success') {
				mchimpMessageArea.removeClass('error').addClass('success').html('<p>Thank you for your interest.</p>').slideDown();
			} else if(resp.result === 'error') {
				mchimpMessageArea.removeClass('success').addClass('error').html('<p>Please enter a valid email address.</p>').slideDown();
			}
		},
		url: "//sandboxapp.us12.list-manage.com/subscribe/post?u=bdb79b2b1fb1fe50b70873fe5&amp;id=17ec5a21fe" //Replace this with your own mailchimp post URL. Don't remove the "". Just paste the url inside "".
	});
}(jQuery));
